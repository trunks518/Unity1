...
public class Buoyancy : MonoBehaviour
{
	...
	private void Start()
	{
		GameObject m_ocean = GameObject.Find("Ocean");
		if(m_ocean == null)
		{
			Debug.Log("AddBuoyancy::Start - Could not find ocean game object");	
			return;
		}
		ocean = m_ocean.GetComponent<Ocean>();
		if (ocean == null) {
		...
		rigidbody.centerOfMass = centerOfMass; //new Vector3(0, -bounds.extents.y * 0f, 0) + transform.InverseTransformPoint(bounds.center);
	...
	{
		return ocean == null ? 0.0f : ocean.SampleHeight(new Vector3(x,0,z));
		//return 0.0f;
	}
	...
	private void OnDrawGizmos()
	{
		...
		Gizmos.DrawSphere (transform.TransformPoint (rigidbody.centerOfMass), 1);
