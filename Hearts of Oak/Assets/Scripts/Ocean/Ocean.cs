...
public class Ocean : MonoBehaviour 
{
	...
	void Start () 
	{
		...
		//if(!GameObject.Find("Ocean Grid Wireframe"))
		//{
	...
	void checkInput() {	
		if (Input.GetMouseButton (0)) {
			...
			if (Physics.Raycast (Camera.main.ScreenPointToRay(Input.mousePosition), out hit)) {
				//make something at this point
				//Instantiate(spawnObject, hit.point, Quaternion.identity);
				//var soItem = spawnObject.AddComponent("AddBuoyancy");
				//Destroy(soItem,5);
				//Bounds bounds = m_oceanMat.bounds;
	...
	void splashAtPoint(int x, int y) {
		//GameObject buoy = new GameObject ();
